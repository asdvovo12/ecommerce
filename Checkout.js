...
import { View, Text, StyleSheet, ScrollView, TouchableOpacity, SafeAreaView, Image, Animated, Alert } from 'react-native';
...
const CheckoutScreen = () => {
    ...
    const { cartItems = [], savedAddress, paymentInfo } = route.params || {};
    const { t, i18n } = useTranslation();
    const { isDarkMode } = useDarkMode();
    const { initPaymentSheet, presentPaymentSheet } = useStripe();
    const [isProcessing, setIsProcessing] = useState(false);

    // Order money settings (owner-configurable). Defaults keep total = items.
    const SHIPPING_FLAT = 0; // e.g. 10 for a flat $10 shipping fee
    const TAX_RATE = 0;      // e.g. 0.14 for 14% VAT

    const proceedGlowAnim = useRef(new Animated.Value(0)).current;
    
    // تأكد من وجود بيانات حين يتم تحميل الشاشة
    useEffect(() => {
        console.log("CheckoutScreen mounted with params:", route.params);
    ...
    useEffect(() => {
        Animated.loop(
            Animated.sequence([
                Animated.timing(proceedGlowAnim, {
                    toValue: 1,
                    duration: 1500,
    ...
    const renderCartItem = ({ item }) => (
        <View style={[styles.cartItemContainer, isDarkMode && styles.cartItemContainerDark]} key={`${item.id}-${item.storage}`}>
            ...
            <Text style={[styles.quantityText, isDarkMode && styles.quantityTextDark]}>x{item.quantity}</Text>
        </View>
    );

    const total = cartItems?.reduce((acc, item) => acc + (item.price * (1 - (item.discount || 0)) * item.quantity), 0) ?? 0;

    const handleEditPayment = () => {
        console.log("Navigating to PaymentScreen with existing info:", paymentInfo);
    ...
    const handleEditAddress = () => {
        console.log("Navigating to ShippingAddress with existing info:", savedAddress);
    ...
    const handleProceedToPayment = async () => {
        ...
        // --- Money breakdown (charged amount always matches what we save) ---
        ...
        const method = paymentInfo.cardType || 'stripe';
        ...
        try {
            ...
            if (method === 'paypal') {
                // --- PayPal: create payment on server, open approval page ---
                const { approvalUrl, paymentId } = await createPayPalPayment(grandTotal, 'USD');
                await WebBrowser.openBrowserAsync(approvalUrl);

                // Confirm with the server that PayPal ACTUALLY approved the payment.
                // Prevents faking a paid order by just closing the approval browser.
                ...
                // --- Card via Stripe PaymentSheet (secure / PCI compliant) ---
                ...
                if (sheetError) {
                    // User cancelled or the payment failed - do NOT save an order.
            ...
            // Email receipt (best-effort; never blocks the success flow).
            ...
            // Payment succeeded + order saved: clear the cart and show orders.
            try { await AsyncStorage.removeItem('cart'); } catch (e) {}

            Alert.alert(
                t('Success'),
                t('alert.orderPlacedSuccess'),
                        ...
                        {cartItems && cartItems.length > 0 ? (
                            <>
                                ...
                                <View style={[styles.section, { marginTop: 20 }]}>
                                    <Text style={[styles.sectionLabel, isDarkMode && styles.sectionLabelDark]}>{t('Shipping address')}</Text>
                                    {savedAddress && (
                                    ...
                                    >
                                        <Text style={[styles.editButtonText, isDarkMode ? styles.whiteText : styles.darkText]}>{t('Edit')}</Text>
                                    </TouchableOpacity>
                                </View>
                                ...
                                {/* قسم معلومات الدفع - تم تعديله */}
                                <View style={[styles.section, { marginTop: 6 }]}>
                                    <Text style={[styles.sectionLabel, isDarkMode && styles.sectionLabelDark]}>{t('Payment method')}</Text>
                                    
                                    {/* عرض معلومات الدفع إذا كانت متوفرة */}
                                    {paymentInfo ? (
                            ...
                            <>
                                <Text style={[styles.emptyCartText, isDarkMode && styles.emptyCartTextDark]}>{t('Your cart is empty.')}</Text>
                ...
                <View style={[styles.footer, isDarkMode && styles.footerDark]}>
                    <View style={styles.bottomBar}>
                        <View style={[styles.orderTotalContainer, isDarkMode && styles.orderTotalContainerDark]}>
                            <Text style={[styles.orderTotalLabel, isDarkMode && styles.orderTotalLabelDark]}>{t('Order total')}</Text>
                            <Text style={[styles.orderTotalAmount, isDarkMode && styles.orderTotalAmountDark]}>${total.toFixed(2)}</Text>
                        </View>
                        <Animated.View style={[
                            ...
                            >
                                <View style={styles.buttonContent}>
                                    <Ionicons name="lock-closed" size={18} color={isDarkMode ? "white" : "#333"} style={styles.lockIcon} />
                                    <Text style={[styles.proceedButtonText, isDarkMode ? styles.whiteText : styles.darkText]}>{isProcessing ? t('Processing...') : t('Proceed to payment')}</Text>
...
const styles = StyleSheet.create({
    ...
    paymentMethodPlaceholderText: {
        fontSize: 14,
        color: '#999',
        fontStyle: 'italic',
    },
    paymentMethodPlaceholderTextDark: {
        color: '#707070',
    },
});
